...
def letterbox_image_cv2_float(image, expected_size):
    """Deprecated: unused. Prefer `iwpod.preprocess.letterbox` (top-left, production)."""
    from loguru import logger
    logger.warning("letterbox_image_cv2_float is deprecated; use iwpod.preprocess.letterbox")
    from iwpod.preprocess import letterbox
    img_u8 = image
    if img_u8.dtype != np.uint8:
        img_u8 = np.clip(np.asarray(image) * (255.0 if np.asarray(image).max() <= 1.5 else 1.0),
                         0, 255).astype(np.uint8)
    canvas, _, _, _ = letterbox(img_u8, expected_size[0] if isinstance(expected_size, tuple) else expected_size)
...
def crop_region(I,label,bg=0.5):
    ...
    if (np.array(outsize) < 0).any():
        raise ValueError(f"crop_region: negative output size {outsize}")
...
def hsv_transform(I, hsv_modifier):
    # I is float32 BGR in [0,1] (see sampler.augment_sample).
    # Old code added in uint8 HSV space and wrapped around.
    # Do the jitter in float32 and clip to valid range.
    I = (np.clip(I, 0.0, 1.0) * 255.0).astype(np.uint8)
    I = cv2.cvtColor(I, cv2.COLOR_BGR2HSV).astype(np.float32)
    I = I + np.asarray(hsv_modifier, dtype=np.float32).reshape(1, 1, 3)
    # OpenCV HSV ranges: H [0,180), S [0,255], V [0,255]
    I[..., 0] = np.clip(I[..., 0], 0, 179)
    I[..., 1:] = np.clip(I[..., 1:], 0, 255)
    I = cv2.cvtColor(I.astype(np.uint8), cv2.COLOR_HSV2BGR).astype(np.float32) / 255.0
